...
use vsdb::{versioned::mapx::MapxVs, BranchName, VersionName, VsMgmt};

fn read_write(c: &mut Criterion) {
    let mut group = c.benchmark_group("** vsdb::versioned::mapx::MapxVs **");
    group
...
fn branch_version_read_write(c: &mut Criterion) {
    ...
    group.bench_function(" branch version read ", |b| {
        b.iter(|| {
            let n = i.fetch_sub(1, Ordering::SeqCst);
            let name = &n.to_be_bytes();
            db.get_by_branch_version(&n, BranchName(name), VersionName(name));
        })
    });
    group.finish();
}
