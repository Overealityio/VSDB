...
fn test_branch_merge() {
    ...
    let val = pnk!(hdr.get_by_branch(&(&2, &2), INITIAL_BRANCH_NAME));
    assert_eq!(val, value);
}
...
fn test_branch_swap() {
    ...
    let val = pnk!(hdr.get_by_branch(&(&1, &1), bn));
...
fn test_branch_truncate() {
    ...
    assert!(hdr.get(&(&1, &1)).is_none());
    assert!(hdr.get(&(&2, &2)).is_none());
}
...
fn test_insert() {
    ...
    (0..max)
        ...
        .for_each(|(i, value)| {
            ...
            assert_eq!(&pnk!(hdr.remove(&(&i, Some(&i)))).unwrap(), &value);
            assert!(hdr.get(gkey).is_none());
        });
}
...
fn test_valueende() {
    ...
    let mut reloaded =
        pnk!(<MapxDkVs<usize, usize, usize> as ValueEnDe>::decode(&dehdr));
...
fn test_emptystr_version() {
    ...
    assert!(pnk!(hdr.insert((1, 1), value)).is_none());
    assert!(hdr.contains_key(key));
    assert_eq!(pnk!(hdr.get(key)), value);
    assert_eq!(pnk!(hdr.remove(&(&1, Some(&1)))).unwrap(), value);
    assert!(hdr.get(key).is_none());
}
...
fn test_version_get() {
    ...
    pnk!(hdr.insert((1, 1), value));
    assert_eq!(hdr.get(key1).unwrap(), value);

    assert_eq!(hdr.remove(&(&2, Some(&2))).unwrap(), None);

    assert_eq!(hdr.remove(&(&1, Some(&1))).unwrap().unwrap(), value);
